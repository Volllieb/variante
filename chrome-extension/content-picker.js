...
;(function () {
  ...
  // --- Overlay -------------------------------------------------------------
  // ponytail: Guard verhindert doppelte Overlays (zuvor erschienen bei
  // schnellem Doppelklick zwei Overlays übereinander).
  function overlay(msg, ok) {
    ...
    if (ok) {
      card.innerHTML =
        ...
        '<div style="font-size:13px;color:rgba(237,237,237,.62);margin-bottom:20px;line-height:1.5">Switch back to Figma to continue.</div>' +
        '<button id="__ab_close_btn" style="display:block;width:100%;padding:12px;border:none;border-radius:10px;background:#ffffff;color:#000000;font-size:15px;font-weight:600;cursor:pointer;transition:opacity .15s;margin-bottom:8px" onmouseover="this.style.opacity=\'0.85\'" onmouseout="this.style.opacity=\'1\'">Close tab</button>' +
