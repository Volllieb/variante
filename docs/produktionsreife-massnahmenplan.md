...
| RA-06 | **Tote Einstellung:** `profiles.notify_on_winner` existiert in DB + `/api/profile`, ist aber in keinem UI erreichbar. Ein Opt-out für die Auto-Promotion fehlt ganz. | Niedrig | **Behoben** — Migration 038 (`profiles.auto_promote_winner`, Default true). Ist der Schalter aus, protokolliert der Winner-Cron den Gewinner, lässt den Test aber `active` — die Kundenseite bleibt unverändert. Neuer Account-Bereich „Experiments" mit beiden Schaltern, „Apply winner"-Button im Results-UI (vorher gab es keinen manuellen Weg zu `status='done'`). |

**Fazit:** Für einen kontrollierten Launch geeignet — `ASSIGN_SECRET` ist gesetzt (RA-01 behoben), und die Auto-Promotion ist seit Migration 038 abschaltbar (RA-06 behoben). Alle Punkte des Re-Audits sind damit erledigt oder als bewusste Design-Grenze dokumentiert (RA-03).
