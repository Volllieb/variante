...
| 07.07.2026 | **Email-Templates designed.** 8 Supabase-Auth-Templates in `ab-tool/emails/`: Confirmation, Magic Link, Reset, Invite, Change Email + 3 Security-Notifications (Password Changed, Email Address Changed, Sign-in Method Removed). Brand-konform: Monochrom, schwarzer Header + Panda-Logo als inline SVG, kein Gradient/Schatten, 480px Card-Layout. Security-Templates mit `--pro` Warn-Badge, ohne CTA. Anleitung in `emails/README.md`. |
