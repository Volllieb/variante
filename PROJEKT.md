...
| Projekt | URL | Methode |
|---|---|---|
| variante | `www.getvariante.com` | Manuell via `vercel deploy --prod` |

**Vercel-Projekte (Team `volllieb1-2106s-projects`):**
- **`variante`** (aktiv, ID `prj_ZTyArxEeWYx6tGbyBE5ESZht0vtr`) — Root Directory = `ab-tool`, Domain `www.getvariante.com`
- **`ab-tool`** (alt, letztes Deployment Error) — wird nicht mehr verwendet, Env-Vars müssen nach `variante` migriert werden

**Vercel-Link:** Repo-Root via `vercel link` auf `variante` gelinkt. `.vercel/` ist gitignored, jeder neue Dev muss einmalig `vercel link` im Root ausführen.

**⚠️ Fehlende Env-Vars (User-Action nötig):** `variante`-Projekt hat KEINE Env-Vars. Müssen manuell vom alten `ab-tool`-Dashboard kopiert werden → https://vercel.com/volllieb1-2106s-projects/variante/settings/environment-variables. Supabase-Redirect-URLs: `www.getvariante.com/auth/callback` muss in Supabase-Auth-Settings ergänzt werden.

**DNS:** Cloudflare (`courtney.ns.cloudflare.com`, `finley.ns.cloudflare.com`). CNAME `www` → `cname.vercel-dns.com`.
...
| Datum | Eintrag |
|---|---|
| 17.07.2026 | **Vercel-Projekt umbenannt: `ab-tool` → `variante`.** Neues Projekt im Team `volllieb1-2106s-projects`, Root Directory = `ab-tool`, Domain `www.getvariante.com`. Altes `ab-tool`-Projekt existiert noch (letztes Deployment Error). ⚠️ **Env-Vars fehlen komplett** im neuen Projekt — Login/Signup/Dashboard aktuell nicht funktionsfähig. User muss Env-Vars manuell vom alten Dashboard kopieren. | **HeroAnimation-Komponente.** GSAP-basierte Looping-Animation (900×560 Bühne) im Hero — 4 Screens: Cursor-Click → Split-View A/B → Conversion-Counter + Winner → Applied. Respects `prefers-reduced-motion: reduce`. Package: `gsap@^3.15.0`. | **landingCopy-Refactoring.** Neue Typ-Architektur: `PLANS: readonly PlanStructure[]` sprachunabhängig (Preise/Links/Featured), `PlanFeature { label, exclusive }`, `PlanCopy { label, sub, cta, features }`. Tupel-Typen (`Tuple3`, `Tuple4`, `Tuple5`) erzwingen gleiche Element-Anzahl in DE/EN — Type-Error bei Abweichung. `platformSentence()` Utility. Alle 60+ Feld-Referenzen in `page.tsx` auf neue API migriert. | **LandingIcons & TechLogos Rewrite.** Icons organisiert nach Use-Case (Pricing/Trust/How-it-works/Agent-Loop/Demo). TechLogos: Simple-Icons CC0-Pfade statt individueller React-Komponenten — `TechLogoMark`-Komponente rendert beliebiges Logo per `path`-Prop. `Palette`/`FigmaIcon` entfernt, `Bot`/`Search`/`PenLine`/`BarChart3`/`Play` hinzugefügt. | **Build grün.** Alle TypeScript-Fehler aus landingCopy-API-Migration behoben. Build-Pflicht eingehalten. |
