...
export default async function TestsPage() {
  ...
  const [profileRes, testsRes, domainsRes, dailyRes] = await Promise.all([
    ...
    supabase
      .from('tests')
      .select('id, name, site_url, status, health_status, health_issues, selector, original_html, site_css, goal, variant_b_html, variant_b_css, variant_b_changes, element_type, visitors_a, visitors_b, conversions_a, conversions_b, winner, created_at, traffic_split, min_visitors, min_uplift')
      .eq('user_id', user.id)
