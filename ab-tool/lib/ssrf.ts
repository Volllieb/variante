...
// Verwendet in /api/domains (CRUD), /api/domains/verify (HTTP-Check) und
// /api/preview bzw. lib/extractPageCode (Hybrid-Onboarding).
...
/** Zentrale Prüfung — auch für Redirect-Ziele und Subressourcen (Stylesheets). */
export function isBlockedHost(hostname: string): boolean {
  return BLOCKED_HOSTS.test(hostname) || BLOCKED_HOSTNAMES.includes(hostname)
}

/** Wirft, wenn eine URL auf einen geblockten Host zeigt oder unparsebar ist. */
